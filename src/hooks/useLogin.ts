...
export function useLogin() {
  ...
  const handleLogin = (event: FormEvent<HTMLFormElement>) => {
    ...
    if (!validUsername()) {
      setError(
        "Username should have 4 characters at least or to be a valid email address",
      );
      return;
    }
    if (!validPassword()) {
    ...
    loginRequest({ username, password, keepLoggedIn })
      ...
      .catch((err) => {
        if (err === "Unauthorized") {
          ...
          setError(
            `Unexpected error on login request: ${err instanceof Error ? err.message : String(err)}`,
          );
        }
      })
      .finally(() => {
  ...
  const handleSignup = (event: FormEvent<HTMLFormElement>) => {
    ...
    signupRequest({ username, email, password }).then((data) => {
