...
export function useAccount() {
  ...
  const [usernameAvailable, setUsernameAvailable] = useState<Available>({
    fetched: false,
    available: false,
  });
  const [emailAvailable, setEmailAvailable] = useState<Available>({
    fetched: false,
    available: false,
  });
  ...
  const handleSubmit = (event: SyntheticEvent<HTMLFormElement>) => {
    ...
    const { fetched: usernameFetched, available: usernameAvbl } =
      usernameAvailable;
    ...
    updateUserDetails({
      ...
      .catch((err) => {
        setError(
          `Error updating user account: ${err instanceof Error ? err.message : String(err)}`,
        );
      });
  };
