...
import {
  restorePassword,
  sendRecoveryEmail,
} from "../../services/RestorePasswordService";
...
export function RestorePasswordPage() {
  ...
  return token ? (
    <RestorePasswordFragment token={token} />
  ) : (
    <SendEmailFragment />
  );
}
...
function SendEmailFragment() {
  ...
  return !summary ? (
    <>
      ...
      <p>
        Enter your address and if you are registered we will send you an email
      </p>
    ...
    <>
      <p>{summary}</p>
      <button onClick={handleReset} style={{ margin: "auto" }}>
        Accept
      </button>
    </>
  );
